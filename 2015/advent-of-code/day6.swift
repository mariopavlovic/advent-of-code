...
private func pointFromString(input: String) -> CGPoint {
...
private func processInput(input: String) -> (action: String, start: CGPoint, end: CGPoint) {
    ...
    words.removeRange(startPointIndex ..< words.endIndex)
    let action = words.joinWithSeparator(" ")
    
    return (action, startPoint, endPoint)
}

private func performAction(action: Action, index: Int) {
    switch action {
    case .TurnOn:
        lights[index] = 1
        brightness[index] += 1
...
private func performInstructions(instructions :[Instruction]) {
