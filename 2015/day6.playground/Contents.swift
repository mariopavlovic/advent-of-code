...
//let input: String = "turn on 887,9 through 959,629\n" +
...
//let input: String = "turn off 225,603 through 483,920\n" +
//                    "toggle 717,493 through 930,875\n" +
//                    "toggle 534,948 through 599,968\n" +
//                    "turn on 522,730 through 968,950\n" +
//                    "turn off 102,229 through 674,529"

let input: String = "turn on 225,603 through 383,720\n"

//let input: String = "turn on 0,0 through 100,4\n" +
//                    "toggle 0,0 through 0,4\n"
...
struct Instruction {
    ...
    init(start: CGPoint, end: CGPoint, input: String) {
        startPoint = start
        endPoint = end
...
func pointFromString(input: String) -> CGPoint {
    let data = input.componentsSeparatedByString(",")
    return CGPointMake(CGFloat((data.first! as NSString).floatValue), CGFloat((data.last! as NSString).floatValue))
}

func processInput(input: String) -> (action: String, start: CGPoint, end: CGPoint) {
    var words = input.componentsSeparatedByString(" ")
    
    //we know that last and last - 2 items are points
    let startPointIndex = words.endIndex - 3
    let endPoint = pointFromString(words.last!)
    let startPoint = pointFromString(words[startPointIndex])

    //now we know that we need to remove everything from start point till the end
    //this will leave only action (first one or two words)
    words.removeRange(Range<Int>(start: startPointIndex, end: words.endIndex))
    let action = words.joinWithSeparator(" ")
    
    return (action, startPoint, endPoint)
}

func performAction(action: Action, index: Int) {
    switch action {
        case .TurnOn:
            lights[index] = 1
        case .TurnOff:
...
func performInstructions(instructions :[Instruction]) {
    for instruction: Instruction in instructions {
        for y in Int(instruction.startPoint.y)...Int(instruction.endPoint.y) {
            for x in Int(instruction.startPoint.x)...Int(instruction.endPoint.x) {
                performAction(instruction.action, index: y * 1000 + x)
            }
        }
    }
}

func numberOfLitLights() -> Int {
    //"+" is short for "$0 + $1"
...
let instructions = inputArray.map { (inputData) -> Instruction in
    let data = processInput(inputData)
...
let result = numberOfLitLights()

print("there are \(result) lit lights")

let diff = NSDate().timeIntervalSinceDate(time)
print("diff \(diff)")
